...
public class CFileUtil {
    ...
    public static boolean deleteDirectory(String dir) {
        ...
        for (int i = 0; i < files.length; i++) {
            // 删除子文件
            if (files[i].isFile()) {
                flag = CFileUtil.deleteFile(files[i].getAbsolutePath());
                if (!flag) {
                    break;
                }
            }
            // 删除子目录
                ...
                flag = CFileUtil.deleteDirectory(files[i]
                        .getAbsolutePath());
                if (!flag) {
                    break;
                }
    ...
    public static void main(String[] args) {
        // 删除单个文件
        String file = "D:\\cblog-blog\\data\\blog\\admin\\webp\\2020\\6\\15\\123.txt";
        CFileUtil.deleteFile(file);
        System.out.println();
