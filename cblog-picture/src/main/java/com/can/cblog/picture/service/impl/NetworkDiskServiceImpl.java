...
import com.can.cblog.base.exception.exceptionType.UpdateException;
...
import com.can.cblog.picture.mapper.NetworkDiskMapper;
import com.can.cblog.picture.service.NetworkDiskService;
import com.can.cblog.picture.service.StorageService;
import com.can.cblog.picture.util.CFileUtil;
import com.can.cblog.picture.util.FeignUtil;
import com.can.cblog.picture.util.MinioUtil;
import com.can.cblog.picture.util.QiniuUtil;
import com.can.cblog.picture.vo.NetworkDiskVO;
import com.can.cblog.utils.StringUtils;
import com.can.cblog.utils.upload.FileUtil;
    ...
    public void deleteFile(NetworkDiskVO networkDiskVO, Map<String, String> qiNiuConfig) {
        ...
        if (SysConf.ONE == networkDisk.getIsDir()) {
            ...
            if (list.size() > 0) {
                ...
                if (isUpdateSuccess) {
                    ...
                    if (EOpenStatus.OPEN.equals(uploadLocal)) {
                        ...
                        // 批量删除本地图片
                        CFileUtil.deleteFileList(fileList);
                    }
                    // 删除七牛云上文件
            ...
            if (EOpenStatus.OPEN.equals(uploadLocal)) {
                ...
                CFileUtil.deleteFile(UPLOAD_PATH + localUrl);
            }
            // 删除七牛云上文件
            if (EOpenStatus.OPEN.equals(uploadQiNiu)) {
